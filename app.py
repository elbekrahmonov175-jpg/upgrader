...
from skin_art import generate_skin_svg
...
def create_app():
    ...
    # ---------- ГРАФИКА СКИНОВ (генерируется, не чужие фото) ----------

    @app.route("/skin-art/<int:skin_id>.svg")
    def skin_art(skin_id):
        skin = db.session.get(Skin, skin_id)
        if not skin:
            return Response(status=404)
        svg = generate_skin_svg(
            weapon_category=skin.weapon_category(),
            full_name=f"{skin.weapon}|{skin.name}",
            rarity_color=skin.rarity_info()["color"],
            wear=skin.wear,
        )
...
def _seed_skins_if_empty():
    if Skin.query.first() is not None:
        return
    for weapon, name, rarity, price, wear in SKINS:
        db.session.add(
            Skin(weapon=weapon, name=name, rarity=rarity, price=price, wear=wear)
        )
    db.session.commit()
