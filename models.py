...
class Skin(db.Model):
    ...
    image_url = db.Column(db.String(500), nullable=True)   # внешняя ссылка на реальное фото скина
    wear = db.Column(db.String(32), nullable=True)         # напр. "Factory New", "Battle-Scarred"
    ...
    def to_dict(self):
        ...
        return {
            ...
            "name": self.name,
            "wear": self.wear,
            "full_name": f"{self.weapon} | {self.name}" + (f" ({self.wear})" if self.wear else ""),
            "price": self.price,
            "rarity": info["key"],
